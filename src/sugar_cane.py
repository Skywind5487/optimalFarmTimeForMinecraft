...
def sugarcane_yield_binom(x: int, p: float) -> float:
    """Binomial distribution model for calculating sugar cane yield rate
    ...
    Returns:
        float: Expected yield rate in items per tick
    """
    ...
    expected = P1 + 2 * P2                # Expected yield
    return expected / x

def sugarcane_yield_poisson(x: int, p: float) -> float:
    """Poisson distribution model for calculating sugar cane yield rate
...
if __name__ == "__main__":
    ...
    calc = rate_calculator(
        binomial_func=sugarcane_yield_binom,
        poisson_func=sugarcane_yield_poisson,
        max_ticks=10**5,
